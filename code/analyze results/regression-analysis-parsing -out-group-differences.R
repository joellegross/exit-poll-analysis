...
for (year in years_to_run) {
  ...
  # Join predicted with ACS summary data
  merged_df <- left_join(predicted_df, acs_df, by = "state") %>%
    drop_na(pct_black)  # Make sure the join succeeded
...
facet_plot <- ggplot(combined_all, aes(x = pct_black, y = predicted, color = region)) +
  ...
  geom_text(
    data = facet_labels,
    mapping = aes(x = 25, y = 0.27, label = label),  # Adjust as needed
    inherit.aes = FALSE,
    size = 4
  ) +
  labs(
